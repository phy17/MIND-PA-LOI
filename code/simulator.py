...
class Simulator:
    ...
    def render_png(self, frame_idx, img_dir):
        render_cfg = self.config.get('render_config', {})
        fig_size = render_cfg.get('figsize', 48)
        dpi = render_cfg.get('dpi', None)
...
#这个函数负责画图
    def render_frame(self, frame_idx, ax):
        ...
        range_3d = 15.0
        font_size = self.config.get('render_config', {}).get('font_size', 35)
        reset_ax(ax)
