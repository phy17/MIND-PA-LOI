...
from planners.mind.baselines import make_supervisor, apply_velocity_bound
...
# === 全局开关：已发表基线监督器（论文对比实验用）===
# None       = 不启用基线（默认，PA-LOI / MIND 行为完全不变）
# 'reachset' = B1 可达集速度约束 (Orzechowski ITSC'18; Koschi & Althoff T-IV'21)
# 'shadow'   = B2 动态阴影跟踪 (Nager, Censi, Frazzoli ICRA'19)
# 启用后：PA-LOI 风险代价被旁路（纯 MIND 规划），检测前端仅用于提取遮挡几何，
# 基线作为规划输出端的速度上界监督器生效；AEB 护盾不受影响（配置对等）。
# 实验脚本用法：planner_module.BASELINE_MODE = 'reachset'
BASELINE_MODE = None
...
class MINDPlanner:
    def __init__(self, config_dir):
        ...
        # 已发表基线监督器（BASELINE_MODE 非 None 时惰性创建，B2 有跨帧状态）
        self.baseline_supervisor = None
        self._last_baseline_bound = float('inf')
    ...
    def plan(self, lcl_smp):
        ...
        # 基线模式 (BASELINE_MODE) 复用同一检测前端提取遮挡几何
        if ENABLE_GHOST_PROBE or BASELINE_MODE is not None:
        ...
        if BASELINE_MODE is not None:
            # 基线模式：PA-LOI 风险代价完全旁路（纯 MIND 规划），
            # 遮挡几何只供输出端的基线监督器消费。
            active_risk_sources = []
        else:
        ...
        # === 已发表基线监督器 (B1 reachset / B2 shadow) ===
        # 在纯 MIND 输出上施加速度上界钳位；位于 AEB 之前，与 PA-LOI 的
        # 作用层级对等（PA-LOI 在 iLQR 内塑形，基线在输出端钳位）。
        if BASELINE_MODE is not None and self.state is not None:
            if self.baseline_supervisor is None or self.baseline_supervisor.name != BASELINE_MODE:
                self.baseline_supervisor = make_supervisor(BASELINE_MODE)
            bound = self.baseline_supervisor.compute_bound(
                risk_sources,
                ego_pos=self.state[:2],
                ego_heading=float(self.state[3]),
                ego_v=float(self.state[2]),
            )
            self._last_baseline_bound = bound
            ret_ctrl, clamped = apply_velocity_bound(ret_ctrl, float(self.state[2]), bound)
            if clamped and DEBUG_LOG_ENABLED:
                print(f"[BASELINE:{BASELINE_MODE}] v={self.state[2]:.2f} m/s > bound={bound:.2f} m/s "
                      f"-> acc clamped to {ret_ctrl[0]:.2f} m/s²")
